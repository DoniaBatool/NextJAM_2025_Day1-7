...
const Cartpage = ({ stockData }:CartpageProps) => {
  
  const { cart, updateQuantity, removeItem } = useCart();
  const [stockInfo, setStockInfo] = useState(stockData || {});

  useEffect(() => {
    setStockInfo(stockData); // Ensure stock data is set when received
  }, [stockData]);
        ...
        {cart.length > 0 ? (
          <>
            ...
            <div className="space-y-6">
              {cart.map((item) => (
                <div key={item.productId} className="grid grid-cols-1 sm:grid-cols-6 gap-4 items-center p-4">
                  <div className="flex flex-col lg:flex-row items-center col-span-2 gap-4">
            ...
            <div className="flex justify-end px-4 mb-8 mt-6">
              <button className="text-white bg-mytext px-6 py-3 rounded-md text-[16px] sm:w-[172px] "onClick={handleCheckout}>
              Checkout</button>
            </div>
          </>
        ) : (
